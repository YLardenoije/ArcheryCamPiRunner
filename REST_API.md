...
curl -X POST http://<pi-ip>:8080/upload -F "file=@target.png"
...
Responses:
- Success: HTTP 302 redirect to /
- Error: HTTP 400 (missing URL), HTTP 404 (camera not found)

Example curl:

curl -X POST http://<pi-ip>:8080/camera_settings \
  -d "url=rtsp://192.168.100.198:554/11" \
  -d "name=Main Camera" \
  -d "role=primary" \
  -d "zoom=22.01" \
  -d "focus=0.00" \
...
- For machine control, prefer /ptz_live for PTZ operations.
