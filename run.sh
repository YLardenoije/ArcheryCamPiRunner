...
start_background() {
    ...
    if [ -n "$WAYLAND_DISPLAY_VALUE" ] || [ -n "$XDG_RUNTIME_DIR_VALUE" ]; then
        PYTHONUNBUFFERED=1 DISPLAY="$DISPLAY_VALUE" XAUTHORITY="$XAUTHORITY_VALUE" WAYLAND_DISPLAY="$WAYLAND_DISPLAY_VALUE" XDG_RUNTIME_DIR="$XDG_RUNTIME_DIR_VALUE" setsid "$PYTHON_BIN" -u "$APP_ENTRY" >> "$LOG_FILE" 2>&1 &
    else
        PYTHONUNBUFFERED=1 DISPLAY="$DISPLAY_VALUE" XAUTHORITY="$XAUTHORITY_VALUE" setsid "$PYTHON_BIN" -u "$APP_ENTRY" >> "$LOG_FILE" 2>&1 &
    fi
    local pid=$!
    echo "$pid" > "$PID_FILE"
...
start_foreground() {
    ...
    if [ -n "$WAYLAND_DISPLAY_VALUE" ] || [ -n "$XDG_RUNTIME_DIR_VALUE" ]; then
        exec env PYTHONUNBUFFERED=1 DISPLAY="$DISPLAY_VALUE" XAUTHORITY="$XAUTHORITY_VALUE" WAYLAND_DISPLAY="$WAYLAND_DISPLAY_VALUE" XDG_RUNTIME_DIR="$XDG_RUNTIME_DIR_VALUE" "$PYTHON_BIN" -u "$APP_ENTRY"
    else
        exec env PYTHONUNBUFFERED=1 DISPLAY="$DISPLAY_VALUE" XAUTHORITY="$XAUTHORITY_VALUE" "$PYTHON_BIN" -u "$APP_ENTRY"
    fi
}
